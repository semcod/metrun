...
def clean_default_tracer() -> None:
    """Ensure the default tracer is clean before and after each test."""
    reset()
    yield
    reset()

def test_full_pipeline_decorator() -> None:
...
def test_full_pipeline_section() -> None:
...
def test_full_pipeline_parent_child_relationship() -> None:
...
def test_full_pipeline_report_output(capsys) -> None:
...
def test_multiple_calls_in_loop() -> None:
