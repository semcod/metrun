...
class TestSuggest:
    ...
    def test_loop_hotspot_suggestions(self) -> None:
        ...
        assert any(
            ("lru_cache" in t or "vectori" in t.lower() or "Numba" in t for t in titles)
        )
    ...
    def test_slow_execution_suggestions(self) -> None:
        ...
        assert tips
        assert any(
            ("profile" in t.detail.lower() or "cache" in t.detail.lower() for t in tips)
        )
    ...
    def test_high_score_appends_extra_suggestions(self) -> None:
        b = _make_bottleneck(
            diagnosis="🔥 loop hotspot", score=_HIGH_SCORE_THRESHOLD + 1.0
        )
    ...
    def test_high_score_includes_scalene_or_viztracer(self) -> None:
        b = _make_bottleneck(
            diagnosis="🐢 slow execution", score=_HIGH_SCORE_THRESHOLD + 1.0
        )
        tips = suggest(b)
    ...
    def test_javascript_catalogue_is_language_aware(self) -> None:
        b = _make_bottleneck(
            diagnosis="🌲 dependency bottleneck", language="javascript"
        )
        tips = suggest(b)
        libraries = {tip.library for tip in tips}
        assert any(
            ("Promise.all" in lib or "worker_threads" in lib for lib in libraries)
        )
        assert not any(
            (
                "functools" in lib or "numpy" in lib or "numba" in lib
    ...
    def test_rust_catalogue_is_language_aware(self) -> None:
        ...
        libraries = {tip.library for tip in tips}
        assert any(
            ("cargo-flamegraph" in lib or "criterion" in lib for lib in libraries)
        )
