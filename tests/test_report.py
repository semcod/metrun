...
def test_report_contains_function_name():
    b = _make_bottleneck(name="slow_query")
    report = generate_report([b])
    assert "slow_query" in report


def test_report_contains_time():
...
def test_report_contains_calls():
...
def test_severity_icon_loop_hotspot():
    b = _make_bottleneck(diagnosis="🔥 loop hotspot")
    report = generate_report([b])
    assert "🔴" in report


def test_severity_icon_dependency_bottleneck():
    b = _make_bottleneck(diagnosis="🌲 dependency bottleneck")
    report = generate_report([b])
    assert "🟠" in report


def test_severity_icon_slow_execution():
    b = _make_bottleneck(diagnosis="🐢 slow execution")
    report = generate_report([b])
    assert "🟡" in report


def test_severity_icon_nominal():
...
def test_dependency_graph_shown_when_children_exist():
...
def test_dependency_graph_hidden_when_show_graph_false():
    b = _make_bottleneck(name="parent", children=["child"])
    report = generate_report([b], show_graph=False)
    assert "Dependency Graph" not in report


def test_dependency_graph_not_shown_when_no_children():
...
def test_summary_contains_top_bottleneck():
    bottlenecks = [
        _make_bottleneck(name="worst", score=9.0),
        _make_bottleneck(name="ok", score=2.0),
    ]
...
def test_report_shows_parent_in_called_by():
...
def test_print_report_produces_output(capsys):
    b = _make_bottleneck()
    print_report([b])
