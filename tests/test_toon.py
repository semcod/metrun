...
def _sample_records() -> dict[str, FunctionRecord]:
    return {'handler': FunctionRecord(name='handler', total_time=0.8, calls=1, children=['slow_query'], parents=[]), 'slow_query': FunctionRecord(name='slow_query', total_time=0.75, calls=100, children=[], parents=['handler'])}
...
class TestGenerateToon:
    ...
    def test_header_contains_metrun(self) -> None:
        result = generate_toon(_sample_bottlenecks(), _sample_records())
        assert result.startswith('# metrun')

    def test_header_contains_date(self) -> None:
        result = generate_toon(_sample_bottlenecks(), _sample_records(), date='2026-04-07')
    ...
    def test_bottlenecks_section_present(self) -> None:
        result = generate_toon(_sample_bottlenecks(), _sample_records())
        assert 'BOTTLENECKS[' in result

    def test_critical_path_section_present(self) -> None:
        records = _sample_records()
        result = generate_toon(_sample_bottlenecks(records), records)
        assert 'CRITICAL-PATH' in result

    def test_critical_path_absent_without_records(self) -> None:
        result = generate_toon(_sample_bottlenecks())
        assert 'CRITICAL-PATH' not in result

    def test_suggestions_section_present(self) -> None:
        result = generate_toon(_sample_bottlenecks(), _sample_records())
        assert 'SUGGESTIONS[' in result

    def test_endpoints_section_present(self) -> None:
        records = _sample_records()
        result = generate_toon(_sample_bottlenecks(records), records)
        assert 'ENDPOINTS[' in result
        assert 'handler' in result

    def test_tree_section_present(self) -> None:
        records = _sample_records()
        result = generate_toon(_sample_bottlenecks(records), records)
    ...
    def test_top_n_limits_output(self) -> None:
        ...
        assert 'bottlenecks: 2' in result

    def test_language_tag_non_python(self) -> None:
        records = {'main': FunctionRecord(name='main', total_time=1.0, calls=1, children=[], parents=[], language='javascript')}
        bns = analyse(records)
...
class TestScanCLI:
    ...
    def test_scan_demo_script(self, tmp_path) -> None:
    ...
    def test_scan_records_file(self, tmp_path) -> None:
        from click.testing import CliRunner
        from metrun.cli import cli
        ...
        result = runner.invoke(cli, ['scan', '--records', str(rec_file), '-o', out_dir])
        assert result.exit_code == 0, result.output
        toon_file = Path(out_dir) / 'metrun.toon.yaml'
        assert toon_file.exists()

    def test_scan_no_args_errors(self) -> None:
        from click.testing import CliRunner
        from metrun.cli import cli
        runner = CliRunner()
