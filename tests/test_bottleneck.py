...
def test_diagnosis_loop_hotspot() -> None:
    records = {
        "hot_loop": _make_record("hot_loop", total_time=0.5, calls=LOOP_THRESHOLD)
    }
    result = analyse(records)
...
def test_diagnosis_dependency_bottleneck() -> None:
    ...
    for child in children:
        records[child] = _make_record(
            child, total_time=0.1, calls=1, parents=["parent"]
        )
    result = analyse(records)
...
def test_score_increases_with_more_children() -> None:
    ...
    many_children = _make_record(
        ...
        total_time=1.0,
        calls=1,
        children=[f"c{i}" for i in range(DEP_THRESHOLD + 2)],
    )
